...
export const Views = {
    ...
    /**
     * Renders the Profile simulated control panel (previously Family Sharing)
     * Features: User Profile Card, Role switcher, Permissions overview, and Family Sharing settings.
     */
    renderProfile(state) {
        const family = db.getFamilyMembers();
        const currentUser = db.getCurrentUser();
        const pending = db.getPendingInvites ? db.getPendingInvites() : [];
        return `<div class="page-view" style="max-width:850px;"><div class="family-layout"><section class="sharing-members-card"><h2 class="detail-section-title">My profile</h2><form id="profile-edit-form"><div style="display:flex;gap:16px;align-items:center;margin:20px 0;"><div id="profile-pic-container" style="width:64px;height:64px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;overflow:hidden;cursor:pointer;">${currentUser.profilePic ? `<img src="${currentUser.profilePic}" style="width:100%;height:100%;object-fit:cover;">` : `<i class="fas fa-user" style="color:white;"></i>`}</div><input id="profile-pic-file" type="file" accept="image/*" hidden><button type="button" id="profile-pic-upload-btn" class="btn-secondary">Change photo</button></div><div class="form-group"><label>Name</label><input id="profile-name" required value="${currentUser.name}"></div><div class="form-group"><label>Email</label><input disabled value="${currentUser.email}"></div><button class="btn-primary" type="submit">Save profile</button></form><button class="btn-secondary" id="profile-signout-btn" style="margin-top:16px;">Sign out</button></section><section class="sharing-members-card"><h2 class="detail-section-title">Family members</h2><p style="color:var(--text-secondary);margin:8px 0 16px;">People with access to your appliance details.</p><div class="members-list">${family.map(member=>`<div class="member-item"><div class="member-info"><strong>${member.name}</strong><span class="member-email">${member.email}</span></div><span class="badge status-success">${member.role}</span></div>`).join('')}${pending.map(invite=>`<div class="member-item"><div class="member-info"><strong>${invite.name || 'Family member'}</strong><span class="member-email">${invite.email}</span></div><span class="badge status-warning">Pending</span></div>`).join('')}</div>${db.isAdmin()?`<form id="family-invite-form" style="margin-top:24px;"><div class="form-group"><label>Family member name</label><input id="family-name" placeholder="Name" required></div><div class="form-group"><label>Family member login email</label><input id="family-email" type="email" placeholder="name@example.com" required></div><button class="btn-primary" type="submit">Add family member</button></form>`:''}</section></div></div>`;
        return `<div class="page-view" style="max-width:850px;"><div class="family-layout"><section class="sharing-members-card"><h2 class="detail-section-title"><i class="fas fa-user-circle"></i> My profile</h2><p style="color:var(--text-secondary);margin:8px 0 20px;">Update the details your family can see.</p><form id="profile-edit-form"><div style="display:flex;gap:16px;align-items:center;margin-bottom:20px;"><div id="profile-pic-container" style="width:64px;height:64px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;overflow:hidden;cursor:pointer;flex:none;">${currentUser.profilePic ? `<img src="${currentUser.profilePic}" style="width:100%;height:100%;object-fit:cover;">` : `<i class="fas fa-user" style="color:white;"></i>`}</div><input id="profile-pic-file" type="file" accept="image/*" hidden><button type="button" id="profile-pic-upload-btn" class="btn-secondary">Change photo</button></div><div class="form-group"><label>Name</label><input id="profile-name" required value="${currentUser.name}"></div><div class="form-group"><label>Email</label><input disabled value="${currentUser.email}"></div><button class="btn-primary" type="submit">Save profile</button></form><button class="btn-secondary" id="profile-signout-btn" style="margin-top:16px;"><i class="fas fa-right-from-bracket"></i> Sign out</button></section><section class="sharing-members-card"><h2 class="detail-section-title"><i class="fas fa-users"></i> Family access</h2><p style="color:var(--text-secondary);margin:8px 0 16px;">Everyone below can view this household’s appliance records.</p><div class="members-list">${family.map(member=>`<div class="member-item"><div class="member-info"><strong>${member.name}</strong><span class="member-email">${member.email}</span></div><span class="badge ${member.role==='Owner'?'status-success':'status-warning'}">${member.role}</span></div>`).join('')}</div>${db.isAdmin()?`<form id="family-invite-form" style="margin-top:24px;"><div class="form-group"><label>Family member’s login email</label><input id="family-email" type="email" placeholder="name@example.com" required></div><button class="btn-primary" type="submit"><i class="fas fa-user-plus"></i> Add family member</button><p style="font-size:12px;color:var(--text-muted);margin-top:10px;">They must create or use an AppliTrack account with this exact email.</p></form>`:''}</section></div></div>`;

        // Define simulated user selector markup here since it is now housed inside the view
        ...
        return `
            <div class="page-view">
                <div style="margin-bottom: 32px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
                    <div>
                        <h2 style="font-family: var(--font-heading); font-size: 28px; font-weight: 800;">My Profile & Access</h2>
                        <p style="color: var(--text-secondary); font-size: 14px;">Manage account roles, test permission scopes, and grant family shares.</p>
                    </div>
                    <button class="btn-secondary" id="profile-reset-db-btn"><i class="fas fa-undo"></i> WIPE & RESET DATA</button>
                </div>

                <div class="family-layout">
                    <!-- Left: Profile details and role switcher -->
                    <div style="display: flex; flex-direction: column; gap: 24px;">
                        <!-- Active User Card -->
                        <div class="sharing-members-card" style="background: linear-gradient(135deg, rgba(99,102,241,0.06), rgba(168,85,247,0.06)); border-color: rgba(99,102,241,0.15);">
                            <h3 class="detail-section-title" style="margin-bottom: 16px;"><i class="fas fa-user-circle"></i> User Session Info</h3>
                            
                            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
                                <div id="profile-pic-container" style="width: 56px; height: 56px; background: var(--primary); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px; overflow: hidden; border: 2px solid var(--primary); cursor: pointer;" title="Click to upload profile photo">
                                    ${currentUser.profilePic 
                                        ? `<img src="${currentUser.profilePic}" style="width: 100%; height: 100%; object-fit: cover;">` 
                                        : `<i class="fas fa-user"></i>`}
                                </div>
                                <div style="flex-grow: 1;">
                                    <h4 style="font-size: 18px; font-weight: 700; margin: 0;">${currentUser.name}</h4>
                                    <span style="font-size: 13px; color: var(--text-secondary); display: block; margin-bottom: 6px;">${currentUser.email}</span>
                                    <input type="file" id="profile-pic-file" accept="image/*" style="display: none;">
                                    <button type="button" class="btn-secondary" id="profile-pic-upload-btn" style="padding: 4px 8px; font-size: 11px; height: auto; display: inline-flex; align-items: center; gap: 4px;">
                            ...
                            <div class="form-group" style="margin-bottom: 0;">
                                <label for="profile-user-select" style="font-weight: 700; font-size: 11px;">TEST A DIFFERENT SIMULATED ROLE:</label>
                                <select id="profile-user-select" class="user-dropdown" style="background: var(--bg-gradient-end); font-weight: 600;">
                                    ${optionsHTML}
                                </select>
                            </div>
                        </div>

                        <!-- Scope Explainer card -->
                        <div class="sharing-members-card">
                            <h3 class="detail-section-title" style="margin-bottom: 16px;"><i class="fas fa-lock"></i> Role Permissions Matrix</h3>
                            <div style="display: flex; flex-direction: column; gap: 12px; font-size: 12px; line-height: 1.5;">
                                <div style="display: flex; gap: 12px; align-items: flex-start; padding-bottom: 12px; border-bottom: 1px solid var(--card-border);">
                                    <span class="badge status-success" style="min-width: 65px; text-align: center;">Owner</span>
                                    <p style="margin: 0; color: var(--text-secondary);"><strong>Full Administration:</strong> View all modules, record/edit specifications, log maintenance operations, grant family sharing keys, and wipe databases.</p>
                                </div>
                                <div style="display: flex; gap: 12px; align-items: flex-start; padding-bottom: 12px; border-bottom: 1px solid var(--card-border);">
                                    <span class="badge status-warning" style="min-width: 65px; text-align: center;">Editor</span>
                                    <p style="margin: 0; color: var(--text-secondary);"><strong>Data Maintainer:</strong> Add/modify appliance specs and log service records. Cannot delete items or change sharing credentials.</p>
                                </div>
                                <div style="display: flex; gap: 12px; align-items: flex-start;">
                                    <span class="badge" style="min-width: 65px; text-align: center; background: rgba(255,255,255,0.05); color: var(--text-primary); border: 1px solid var(--card-border);">Viewer</span>
                                    <p style="margin: 0; color: var(--text-secondary);"><strong>Read-Only Access:</strong> View dashboard telemetry, inventory statuses, and QR scan pages. Modals, delete tags, and setting panels are disabled.</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Right: Sharing access settings -->
                    <div style="display: flex; flex-direction: column; gap: 24px;">
                        <div class="sharing-members-card">
                            <h3 class="detail-section-title"><i class="fas fa-users"></i> Family Sharing Registry</h3>
                            <p style="font-size: 13px; color: var(--text-secondary); margin: 6px 0 16px;">Grant access keys to extra family members.</p>
                            
                            <div class="members-list" style="margin-top: 0; margin-bottom: 24px;">
                                ${family.map(member => {
                                    const isCurrentUser = db.getCurrentUser().id === member.id;
                                    return `
                                        <div class="member-item">
                                            <div style="display: flex; align-items: center; gap: 12px;">
                                                <div style="width: 36px; height: 36px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; font-size: 14px; overflow: hidden; border: 1px solid var(--card-border); flex-shrink: 0;">
                                                    ${member.profilePic 
                                                ...
                                                <div class="member-info">
                                                    <span class="member-name" style="font-weight: 600;">
                                                        ${member.name} ${isCurrentUser ? '<span style="color:var(--primary); font-size:11px;">(You)</span>' : ''}
                                                    </span>
                                                    <span class="member-email" style="font-size: 11px;">${member.email}</span>
                                                </div>
                                            </div>
                                            <div style="display: flex; align-items: center; gap: 12px;">
                                                <span class="badge ${member.role === 'Owner' ? 'status-success' : member.role === 'Editor' ? 'status-warning' : 'status-success'}" style="background: rgba(255,255,255,0.05); color: var(--text-primary); border: 1px solid var(--card-border); padding: 2px 6px; font-size: 10px;">
                                                    ${member.role}
                                                </span>
                                                ${db.isAdmin() && !isCurrentUser ? `
                                                    <button class="btn-danger family-delete-btn" data-id="${member.id}" style="padding: 4px 8px; font-size: 11px;"><i class="fas fa-user-minus"></i></button>
                                                ` : ''}
                                            </div>
                                        </div>
                                    `;
                            ...
                            <h4 style="font-size: 14px; font-weight: 700; margin-bottom: 12px; color: var(--text-primary); border-top: 1px solid var(--card-border); padding-top: 20px;"><i class="fas fa-user-plus"></i> Grant Access</h4>
                            <form id="profile-add-family-form" style="display: flex; flex-direction: column; gap: 16px;">
                                <div class="form-group" style="margin-bottom: 0;">
                                    <label for="fam-name" style="font-size: 11px;">FULL NAME</label>
                                    <input type="text" id="fam-name" placeholder="E.g., John Doe" required ${!db.isAdmin() ? 'disabled' : ''}>
                                </div>
                                <div class="form-group" style="margin-bottom: 0;">
                                    <label for="fam-email" style="font-size: 11px;">EMAIL ADDRESS</label>
                                    <input type="email" id="fam-email" placeholder="E.g., john@gmail.com" required ${!db.isAdmin() ? 'disabled' : ''}>
                                </div>
                                <div class="form-group" style="margin-bottom: 0;">
                                    <label for="fam-role" style="font-size: 11px;">ACCESS ROLE</label>
                                    <select id="fam-role" ${!db.isAdmin() ? 'disabled' : ''}>
                                        <option value="Viewer">Viewer (Read-only)</option>
                                        <option value="Editor">Editor (Can edit specs / log services)</option>
                                        <option value="Owner">Co-Owner (Full admin rights)</option>
                                    </select>
                                </div>
                                
                                <button type="submit" class="btn-primary" style="margin-top: 8px; width: 100%; justify-content: center;" ${!db.isAdmin() ? 'disabled' : ''}>
                        ...
                        <div class="sharing-members-card" style="margin-top: 24px;">
                            <h3 class="detail-section-title" style="margin-bottom: 12px;"><i class="fas fa-database"></i> Supabase Cloud Sync</h3>
                            <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 16px;">
                                Plug in your Supabase project credentials to activate real-time server database synchronization.
                            </p>
                            ...
                            ${dbConfig.isConnected() ? `
                                <div style="display: flex; align-items: center; gap: 8px; margin-top: 16px; font-size: 12px; color: var(--success); font-weight: 600;">
                                    <i class="fas fa-circle" style="font-size: 8px;"></i> Cloud Sync Connected
                                </div>
                            ` : `
                                <div style="display: flex; align-items: center; gap: 8px; margin-top: 16px; font-size: 12px; color: var(--warning); font-weight: 600;">
                                    <i class="fas fa-circle" style="font-size: 8px;"></i> Running in Local Simulation Mode
                                </div>
                            `}
                        </div>
                    </div>
